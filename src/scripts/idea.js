    ...
    for (i = 0; i < result.ideas.length; i ++) {
        ...
        <p id="${id}-idea">
            ...
            <span class="name">
                ...
                <span id="${id}-idea-edit" style="display: none">
                    <input type="text" id="${id}-idea-edit-name" onkeydown="editIdeaInput(event, '${id}')" placeholder="New Idea Name">
                    <button onclick="displayCloseEditIdea('${id}')"><svg xmlns="http://www.w3.org/2000/svg" height="21px" viewBox="0 -960 960 960" width="21px" fill="#e3e3e3"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"/></svg></button>
                    <button onclick="editIdea('${id}')"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#e3e3e3"><path d="M840-680v480q0 33-23.5 56.5T760-120H200q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h480l160 160Zm-80 34L646-760H200v560h560v-446ZM480-240q50 0 85-35t35-85q0-50-35-85t-85-35q-50 0-85 35t-35 85q0 50 35 85t85 35ZM240-560h360v-160H240v160Zm-40-86v446-560 114Z"/></svg></button>
                </span>
            </span>
            <span id="${id}-edit" style="display: none;" class="center">
                <svg onclick="displayEditIdea('${id}')" xmlns="http://www.w3.org/2000/svg" height="17px" viewBox="0 -960 960 960" width="17px" fill="#e3e3e3"><path d="M200-200h57l391-391-57-57-391 391v57Zm-80 80v-170l528-527q12-11 26.5-17t30.5-6q16 0 31 6t26 18l55 56q12 11 17.5 26t5.5 30q0 16-5.5 30.5T817-647L290-120H120Zm640-584-56-56 56 56Zm-141 85-28-29 57 57-29-28Z"/></svg>
            </span>
        ...
        <div id="${id}-subidea" class="subidea" style="display: none;">
            <div id="${id}-subidea-input" class="input" style="display: none;">
                <input type="text" id="${id}-subidea-input-txt" onkeydown="subIdeaInput(event, '${id}')" placeholder="Sub-Idea">
...
function displayEditIdea(id) {
    ...
    document.getElementById(`${id}-idea-edit-name`).focus();
}

function displayCloseEditIdea(id) {
    document.getElementById(`${id}-idea-name`).style = "display: flex;"
    document.getElementById(`${id}-idea-edit`).style = "display: none"
    document.getElementById(`${id}-idea-edit-name`).value = ""
}

function editIdeaInput(event, id) {
    if (event.key == 'Enter')
        editIdea(id)
}

async function editIdea(id) {
    newName = document.getElementById(`${id}-idea-edit-name`).value

    result = await window.electronAPI.editIdea(id, newName)
    
    if (result == 200) {
        displayCloseEditIdea(id)

        document.getElementById(`${id}-idea-name`).innerText = newName;
    }
